...
public class EasterEggAnimationFx {
    ...
    public static void playFlash(Node node) {
        ...
        try {
            URL soundUrl = EasterEggAnimationFx.class.getResource("/sounds/flash.mp3");
            if (soundUrl != null) {
                AudioClip flashSound = new AudioClip(soundUrl.toExternalForm());
                flashSound.play(); // Le son démarre immédiatement
            } else {
                System.out.println("Fichier son introuvable : /sounds/flash.mp3");
            }
        } catch (Exception e) {
